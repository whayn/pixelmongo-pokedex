...
import { createSearchResult } from '../../utils/utils'
...
module.exports = {
	options: {
		name: "search-info",
		description: "Search infos about a pokemon",
		options: [
	...
	async execute({ client, interaction }: { client: Client, interaction: CommandInteraction }) {
		try {
			...
			if (!pokemons[0]) return interaction.editReply(Notifier.error("Le nom d'item entré est invalide (ex: minecraft:dirt)"))
			...
			if (!formatedPokemons[0].drop) return interaction.editReply(Notifier.error("Le nom d'item entré est invalide (ex: minecraft:dirt)"))

			const researchResultsEmbed = createSearchResult(formatedPokemons, ressource, client)
